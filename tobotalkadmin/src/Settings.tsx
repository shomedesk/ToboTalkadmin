...
export default function Settings() {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "admin_config", "config"), (snap) => {
      if (snap.exists()) {
        setSettings(prev => ({ ...prev, ...snap.data() }));
      }
      setLoading(false);
    }, (error) => {
      console.error("Settings: Admin Config error:", error);
      setLoading(false);
    });
    return () => unsub();
  }, []);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await setDoc(doc(db, "admin_config", "config"), finalSettings);
